...
def detect_with_yoloface(vision_frame: VisionFrame, face_detector_size: str) -> Tuple[List[BoundingBox], List[FaceLandmark5], List[Score]]:
    ...
    def adjust_coordinates(bounding_boxes, landmarks, angle, img_shape):
        # Adjust bounding box and landmark coordinates back to the original direction
        adjusted_bounding_boxes = []
        adjusted_landmarks = []
        
        for bbox in bounding_boxes:
            if angle == 90:
                bbox = np.array([img_shape[1] - bbox[1], bbox[0], img_shape[1] - bbox[3], bbox[2]])
            elif angle == 180:
                bbox = np.array([img_shape[1] - bbox[2], img_shape[0] - bbox[3], img_shape[1] - bbox[0], img_shape[0] - bbox[1]])
            elif angle == 270:
                bbox = np.array([bbox[1], img_shape[0] - bbox[2], bbox[3], img_shape[0] - bbox[0]])
            adjusted_bounding_boxes.append(bbox)
        
        for landmark in landmarks:
            adjusted_landmark = []
            for point in landmark:
                if angle == 90:
                    adjusted_landmark.append([img_shape[1] - point[1], point[0]])
                elif angle == 180:
                    adjusted_landmark.append([img_shape[1] - point[0], img_shape[0] - point[1]])
                elif angle == 270:
                    adjusted_landmark.append([point[1], img_shape[0] - point[0]])
                else:
                    adjusted_landmark.append(point)
            adjusted_landmarks.append(np.array(adjusted_landmark))
        
        return adjusted_bounding_boxes, adjusted_landmarks

    face_detector = get_face_analyser().get('face_detectors').get('yoloface')
    face_detector_width, face_detector_height = unpack_resolution(face_detector_size)
    directions = [0,180]  # north, east, south, west
    ...
    for direction in directions:
        temp_vision_frame = rotate_image(vision_frame, direction)
        resized_frame = resize_frame_resolution(temp_vision_frame, (face_detector_width, face_detector_height))
        ratio_height = vision_frame.shape[0] / resized_frame.shape[0]
        ratio_width = vision_frame.shape[1] / resized_frame.shape[1]
        
        detect_vision_frame = prepare_detect_frame(resized_frame, face_detector_size)
        with thread_semaphore():
            detections = face_detector.run(None, {
                face_detector.get_inputs()[0].name: detect_vision_frame
            })
        ...
        keep_indices = np.where(score_raw > facefusion.globals.face_detector_score)[0]
        ...
        if keep_indices.any():
            ...
            face_landmarks = [np.array(face_landmark_5.reshape(-1, 3)[:, :2]) for face_landmark_5 in face_landmark_5_raw]
            scores = score_raw.ravel().tolist()
            
            adjusted_bounding_boxes, adjusted_landmarks = adjust_coordinates(bounding_boxes, face_landmarks, direction, vision_frame.shape)
            all_bounding_boxes.extend(adjusted_bounding_boxes)
            all_landmarks.extend(adjusted_landmarks)
            all_scores.extend(scores)

    return all_bounding_boxes, all_landmarks, all_scores
